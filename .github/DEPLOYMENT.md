...
### Секреты для базы данных PostgreSQL (опционально, есть значения по умолчанию):

| Имя секрета | Значение | Пример | По умолчанию |
|------------|----------|--------|--------------|
| `DB_HOST` | Хост PostgreSQL | `postgres` | `postgres` |
| `DB_PORT` | Порт PostgreSQL | `5432` | `5432` |
| `DB_NAME` | Имя базы данных | `receipty_bot` | `receipty_bot` |
| `DB_USER` | Пользователь PostgreSQL | `postgres` | `postgres` |
| `DB_PASSWORD` | Пароль PostgreSQL | `your_secure_password` | `postgres` |

**Примечание:** Если секреты базы данных не указаны, будут использованы значения по умолчанию. Рекомендуется установить `DB_PASSWORD` для безопасности.
...
1. При пуше в ветку `main` или `master`:
   ...
   - Останавливает старые контейнеры (bot и postgres)
